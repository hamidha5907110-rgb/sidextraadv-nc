...
def register_handlers(bot: telebot.TeleBot, state: BotState, label: str):
    ...
    def send_menu(message):
        ...
        markup.add(
            telebot.types.InlineKeyboardButton("🔄 Refresh", callback_data=f"refresh_{chat_id}"),
            telebot.types.InlineKeyboardButton("📊 Status", callback_data=f"status_{chat_id}")
        )
        markup.add(
    ...
    def host_ub_callback(call):
        if not admin_only(call.message):
            bot.answer_callback_query(call.id, "⛔ Not authorized")
            return
        bot.answer_callback_query(call.id, "📱 Starting login...")
        start_userbot_login(call.from_user.id, bot, call.message)

    @bot.callback_query_handler(func=lambda call: call.data == "stop_ub")
    def stop_ub_callback(call):
        if not admin_only(call.message):
            bot.answer_callback_query(call.id, "⛔ Not authorized")
            return
        if ub_client is None:
            bot.answer_callback_query(call.id, "⛔ Userbot is not running.")
            return
        bot.answer_callback_query(call.id, "⏳ Stopping...")
        success = stop_userbot()
        if success:
            bot.reply_to(call.message, "✅ Userbot stopped.")
        else:
            bot.reply_to(call.message, "❌ Error stopping userbot.")
