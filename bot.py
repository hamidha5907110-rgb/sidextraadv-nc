...
def process_login_step(user_id, text, bot, message):
    ...
    if step == "phone":
        ...
        try:
            ...
            # Connect and keep the connection alive throughout the auth flow
            client.connect()
        ...
        try:
            # Client is still connected from previous step
            signed_in = client.sign_in(session["phone"], session["sent_code"].phone_code_hash, otp)
            
            # Login successful and no 2FA required
            session["step"] = "done"
            session_string = client.export_session_string()
            client.disconnect() # Auth flow complete, safely disconnect builder client
            
            # Now start the userbot with the session string
            success = start_userbot(session_string, user_id)
            if success:
                bot.reply_to(message, "✅ Hoster deployed successfully! You can now use /ubspam, /ubnc, /ubdc.")
            else:
                bot.reply_to(message, "❌ Failed to start hoster. Check logs.")
            del login_sessions[user_id]
            
        except SessionPasswordNeeded:
            session["step"] = "2fa"
            bot.reply_to(message, "🔐 Two‑factor authentication is enabled. Please enter your 2FA password:")
            # Do NOT disconnect yet, connection needed for checking password
        ...
        try:
            ...
            success = start_userbot(session_string, user_id)
            if success:
                bot.reply_to(message, "✅ Hoster deployed successfully!")
            else:
                bot.reply_to(message, "❌ Failed to start hoster.")
            del login_sessions[user_id]
        except Exception as e:
...
def start_userbot(session_string: str, user_id: int):
    ...
    with ub_lock:
        if ub_client is not None:
            logger.warning("Hoster already running. Stopping old one first.")
            stop_userbot()
        try:
            app = pyrogram.Client(
                f"userbot_{user_id}",
            ...
            for cid, desc in list(ub_state.dc_descs.items()):
                if ub_state.dc_flags.get(cid, False):
                    ...
                    t.start()
            logger.info("✅ Hoster started successfully.")
            save_all_states()
            return True
        except Exception as e:
            logger.error(f"❌ Failed to start hoster: {e}")
            ub_client = None
            return False
...
def stop_userbot():
    ...
    with ub_lock:
        ...
        try:
            ...
            ub_client = None
            save_all_states()
            logger.info("✅ Hoster stopped.")
            return True
        except Exception as e:
            logger.error(f"❌ Error stopping hoster: {e}")
            return False
...
def get_ub_status():
    ...
    try:
        me = ub_client.get_me()
        return f"✅ Active (@{me.username or me.first_name})"
    except:
...
roast_list = [
    ...
    "🤣 Teri thinking 2G speed pe chal rahi duniya 5G mein hai 📡😹",
...
fire_list = [
    ...
    "🔥 Beta fire se bach ke gaya toh winner — nahi gaya toh obvious 💀🔥",
...
devil_list = [
    ...
    "😈 Devil mein aake dekha — tu deserving nahi challenge ka 💀⚡",
...
ghost_list = [
    ...
    "👻 GHOST PRESENCE — tu feel kar sakta par dekh nahi sakta 💀😈",
    ...
    "👻 GHOST VANISH — gaya aur teri legacy chhod gaya — wrong reasons se 🔥💀",
...
legend_list = [
    ...
    "👑 LEGEND COMPLETE — teri story: cautionary tale ban contemplation 😎⚡",
...
nuke_list = [
    ...
    "☢️ Nuke activated — teri har reply waste 💀😈",
...
def build_menu_text(label: str, state: BotState, chat_id: int) -> str:
    """Premium, girly, dynamic HTML dashboard with all feature statuses."""
    def yn(v): return "🟢 <b>𝗢𝗡</b>" if v else "🔴 <b>𝗢𝗙𝗙</b>"
    def ms(v): return f"⚡ {v*1000:.0f}ms" if v else "—"

    # Status grabs
    spam_on = state.spam_flags.get(chat_id, False)
    spam_delay = state.spam_delay.get(chat_id, 1.0)
    
    nc_on = state.nc_flags.get(chat_id, False)
    nc_delay = state.nc_delay.get(chat_id, 2.0)
    
    dc_on = state.dc_flags.get(chat_id, False)
    dc_delay = state.dc_delay.get(chat_id, 3.0)
    
    ad_count = len(state.auto_delete.get(chat_id, set()))
    react = state.auto_react.get(chat_id, "OFF")
    sub_count = len(state.subadmins)
...
💣 <b>𝗦𝗣𝗔𝗠 𝗘𝗡𝗚𝗜𝗡𝗘</b> [ {yn(spam_on)} ]
├ <b>Speed:</b> {ms(spam_delay)}
├ <code>/spam &lt;msg&gt;</code> — Start Spam
├ <code>/spamoff</code> — Stop Spam
└ <code>/spam delay &lt;ms&gt;</code> — Set Speed

🌷 <b>𝗡𝗔𝗠𝗘 𝗖𝗛𝗔𝗡𝗚𝗘𝗥 (𝗡𝗖)</b> [ {yn(nc_on)} ]
├ <b>Speed:</b> {ms(nc_delay)}
├ <code>/nc &lt;name&gt;</code> — Start NC
├ <code>/ncoff</code> — Stop NC
└ <code>/nc delay &lt;ms&gt;</code> — Set Speed

🦋 <b>𝗗𝗘𝗦𝗖 𝗖𝗛𝗔𝗡𝗚𝗘𝗥 (𝗗𝗖)</b> [ {yn(dc_on)} ]
├ <b>Speed:</b> {ms(dc_delay)}
├ <code>/dc &lt;desc&gt;</code> — Start DC
├ <code>/dcoff</code> — Stop DC
└ <code>/dc delay &lt;ms&gt;</code> — Set Speed

🍓 <b>𝗔𝗨𝗧𝗢 𝗗𝗘𝗟𝗘𝗧𝗘𝗥</b>
├ <b>Active Targets:</b> {ad_count} user(s)
├ <code>/auto_delete &lt;id&gt;</code> — Add Target
└ <code>/auto_delete off</code> — Stop all

💖 <b>𝗦𝗨𝗕𝗔𝗗𝗠𝗜𝗡𝗦</b>
├ <b>Total Subadmins:</b> {sub_count}
├ <code>/addsubadmin @user</code>
├ <code>/removesubadmin @user</code>
└ <code>/listsubadmin</code>

🌺 <b>𝗘𝗫𝗧𝗥𝗔 𝗙𝗘𝗔𝗧𝗨𝗥𝗘𝗦</b>
├ <b>Auto React:</b> {react}
├ <code>/react &lt;emoji&gt;</code> — Enable React
├ <code>/stopreact</code> — Disable React
└ <code>/status</code> — System Overview

━━━━━━━━━━━━━━━━━━━━━━━━━━
📌 <b>𝗘𝗫𝗧𝗘𝗡𝗗𝗘𝗗 𝗠𝗘𝗡𝗨𝗦:</b>
<code>/menu1</code> → Admin & Mute Controls
<code>/menu2</code> → Raid Engine (Original)
<code>/menu3</code> → Spam Engine & Texts
"""
